...
if Lora_Flag == True:
    ...
    mode_save_path = "/home/data2/haungqiang/code/defect/checkpoints/Facade_check/latest_net_BA.pth"
    model.load_state_dict(torch.load(mode_save_path))
...
for i, data in enumerate(dataset):
    ...
    # 1. **多尺度特征增强**（提升缺陷区域对比度）
    def enhance_multiscale(error_maps):
        enhanced = []
        for emap in error_maps:
            # 高斯金字塔下采样再上采样，增强多尺度特征
            emap_low = cv2.GaussianBlur(emap, (5, 5), 2)
            emap_high = emap - emap_low
            enhanced.append(emap_high * 2 + 0.5)  # 增强高频分量
        return enhanced


    enhanced_errors = enhance_multiscale(error_maps)


    # 2. **动态权重分配**（根据误差图的区分度自动分配权重）
    def compute_weights(error_maps):
        weights = []
        for emap in error_maps:
            # 通过信息熵评估特征丰富度
            hist = np.histogram(emap * 255, bins=256, range=(0, 255))[0]
            hist = hist / hist.sum()
            e_entropy = entropy(hist)
            # 熵值越大说明信息量越大，权重越高
            weights.append(e_entropy)
        weights = np.array(weights)
        return weights / weights.sum()


    weights = compute_weights(enhanced_errors)

    # 3. **空间注意力融合**
    final_map = np.zeros_like(enhanced_errors[0])
    for i in range(5):
        # 为每个像素生成注意力权重（误差值越大关注度越高）
        attention = np.exp(enhanced_errors[i] * 3)
        attention /= attention.max()
        final_map += enhanced_errors[i] * weights[i] * attention


    # 4. **自适应阈值分割**
    def adaptive_threshold(fused_map):
        # 基于Otsu方法自动确定二值化阈值
        fused_8bit = (fused_map * 255).astype(np.uint8)
        bilateral = cv2.bilateralFilter(fused_8bit, 15, 300, 25)
    ...
    plt.colorbar()
    plt.title('Fused Defect Probability Map')
    plt.axis('off')
    save_path = os.path.join(web_dir, f'sample_{name}_fused_result.png')
    plt.savefig(save_path, bbox_inches='tight', dpi=300)
    plt.close()

    defect_mask = adaptive_threshold(final_map)
    cv2.imwrite(os.path.join(image_dir, f'sample_{name}_defect_mask.png'), defect_mask)

    print(defect_mask.min(), defect_mask.max())  # 查看预测 mask 值范围
    print(label_mask.min(), label_mask.max())  # 查看标签 mask 值范围

    # 确保预测和标签尺寸一致
    assert defect_mask.shape == label_mask.shape[:2], "预测与标签尺寸不匹配"

    # 展平为1D数组（0或1）
    pred_flat = (defect_mask.flatten() > 128).astype(np.uint8)
