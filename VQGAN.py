...
class VQGAN(nn.Module):
    def __init__(self):
        ...
        quant_lis = [
            nn.Conv2d(channel, channel, 1) for channel in channels
        ]
    ...
    def forward(self, imgs, info=None, KD =False):
        ...
        for i, quant_conv in enumerate(self.quant_convs):
            ...
            # 计算每个位置的 L2 量化误差
            error_map = torch.norm(quant_conv_encoded_images - codebook_mapping, p=2, dim=1)  # [B, H, W]
            quant_error_maps.append(error_map.detach().cpu())
            post_quant_conv_mapping = self.post_quant_convs[i](codebook_mapping)
            x_lis.append(post_quant_conv_mapping)
            loss.append(q_loss)  

            ref_unquant.append(quant_conv_encoded_images)
            ref_quant.append(codebook_mapping)
        ...
        if KD == False:
            #return decoded_images, codebook_indices, q_loss,ref_unquant,ref_quant #train
            return decoded_images, codebook_indices, q_loss, ref_unquant, ref_quant, quant_error_maps #test
        else:
