...
def visualize_pixel_diff(img1, img2, title1="Image 1", title2="Image 2", save_path=None):
    ...
    # 创建单个子图并设置尺寸
    plt.figure(figsize=(10, 8))  # 修改图像尺寸

    # 绘制差异热力图（jet颜色映射）
    plt.imshow(pixel_diff, cmap='jet', vmin=0, vmax=0.5)  # 调整vmax为数据范围
    #plt.title("Pixel-wise L1 Difference")  # 设置标题
    plt.axis('off')  # 隐藏坐标轴
...
if Lora_Flag == True: 
    ...
    mode_save_path = "/home/data2/haungqiang/code/defect/checkpoints/crack_check/latest_net_BA.pth"
    model.load_state_dict(torch.load(mode_save_path))   
...
for i, data in enumerate(dataset):
    ...
    # def optimize_weights(error_maps, resolution_factors=[2.0, 1.5, 1.0, 0.8]):
    #     """
    #     自适应权重优化函数
    #     Args:
    ...
    #
    #     # 生成伪缺陷区域的步骤
    #     def generate_pseudo_mask(errors):
    #         from skimage.filters import threshold_otsu
    #         from skimage.morphology import remove_small_objects, binary_opening
    #
    #         # 多层级融合
    #         fused = np.zeros_like(errors[0])
    #         for err in errors[:4]:
    #             normalized_err = (err - np.min(err)) / (np.max(err) - np.min(err) + 1e-8)
    #             fused += normalized_err
    #
    #             # 改进的动态阈值
    #         q = 95 if np.mean(fused) > 0.25 else 99  # 高噪声时提高阈值
    #         thresh = np.percentile(fused, q)
    #         mask = fused > thresh
    #
    #         # 自适应形态学处理
    #         min_size = int(fused.size * 0.0003)  # 按图像尺寸调整
    #         mask = remove_small_objects(mask, min_size=max(10, min_size))
    #         return mask
    #
    #     mask = generate_pseudo_mask(error_maps)
    #     # 计算各层在缺陷区域的响应强度
    #     responses = []
    ...
    # 在权重分配前插入以下代码
    def calculate_adaptive_weights(error_maps):
        """基于各误差图的统计特征动态分配权重"""
        weights = np.zeros(5)

        # 1. 深层特征偏好（Layer4方差越大权重越高）
        layer_var = [np.var(em) for em in error_maps[:4]]
        weights[:4] = np.array(layer_var) / sum(layer_var) * 0.9  # 90%权重分配给Codebook

        # 2. 重构误差的对比度敏感度
        recon_contrast = np.max(error_maps[4]) - np.min(error_maps[4])
        weights[4] = min(recon_contrast * 2, 0.1)  # 不超过10%

        # 3. 空间一致性调整（抑制均匀区域的噪声）
        for i in range(4):
            grad = np.abs(cv2.Sobel(error_maps[i], cv2.CV_64F, 1, 1))
            weights[i] *= np.clip(np.mean(grad) * 3 , 0.7, 0.8)
    ...
    # 替换原有固定权重
    weights = calculate_adaptive_weights(error_maps)
    ...
    # 展平为1D数组（0或1）
    # ======
    pred_flat = (pred_mask.flatten() > 128).astype(np.uint8) *255
    pred_image = pred_flat.reshape(pred_mask.shape)  # 保持原始形状
    # 保存可视化结果
    pred_visual_path = os.path.join(image_dir, f'sample_{name}_pred_visual.png')
    cv2.imwrite(pred_visual_path, pred_image)
    #=========
    #pred_flat = (pred_mask.flatten() > 128).astype(np.uint8)
    label_flat = (label_mask.flatten() > 128).astype(np.uint8) *255
    label_image = label_flat.reshape(label_mask.shape)  # 保持原始形状
    label_visual_path = os.path.join(image_dir, f'sample_{name}_label_visual.png')
    cv2.imwrite(label_visual_path, label_image)
    # 计算TP, FP, TN, FN
